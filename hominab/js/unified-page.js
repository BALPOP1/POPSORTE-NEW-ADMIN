...
window.UnifiedPage = (function() {
    ...
    async function renderEntries() {
        ...
        if (validationResults) {
            ...
            validationResults.results.forEach(v => {
                // Support both old (v.ticket) and new (direct entry) structures
                const ticketNumber = v.ticket?.ticketNumber || v.ticketNumber;
                if (ticketNumber) {
                    validationMap.set(ticketNumber, v);
                }
                // Support both old (v.status) and new (v.validity) structures
                const status = v.validity || v.status;
    ...
    function applyEntriesFilters() {
        ...
        switch (sortBy) {
            ...
            case 'cutoff-yes':
                result.sort((a, b) => {
                    const aVal = validationMap.get(a.ticketNumber);
                    const bVal = validationMap.get(b.ticketNumber);
                    const aCutoff = (aVal?.cutoffFlag || aVal?.isCutoff) ? 1 : 0;
                    const bCutoff = (bVal?.cutoffFlag || bVal?.isCutoff) ? 1 : 0;
            ...
            case 'status-valid':
                result.sort((a, b) => {
                    const aVal = validationMap.get(a.ticketNumber);
                    const bVal = validationMap.get(b.ticketNumber);
                    const aStatus = aVal?.validity || aVal?.status || 'UNKNOWN';
                    const bStatus = bVal?.validity || bVal?.status || 'UNKNOWN';
                    const aValid = aStatus === 'VALID' ? 1 : 0;
                    const bValid = bStatus === 'VALID' ? 1 : 0;
            ...
            case 'status-invalid':
                result.sort((a, b) => {
                    ...
                    const aStatus = aVal?.validity || aVal?.status || 'UNKNOWN';
                    const bStatus = bVal?.validity || bVal?.status || 'UNKNOWN';
                    const aInvalid = aStatus === 'INVALID' ? 1 : 0;
                    const bInvalid = bStatus === 'INVALID' ? 1 : 0;
    ...
    function renderEntriesTable() {
        ...
        try {
            ...
            // ✅ GET VALIDATION FROM validationMap
            const validation = validationMap.get(entry.ticketNumber);
            
            // Support both old and new validation structure
            const status = validation?.validity || validation?.status || 'UNKNOWN';
            ...
            // Status badge based on validation result
            let statusBadge = '';
            switch (status) {
                case 'VALID':
                    ...
                    break;
                case 'INVALID':
                    const reasonText = validation?.invalidReasonCode 
                        ? RechargeValidator.getReasonCodeText(validation.invalidReasonCode)
                        : reasonCode;
                    statusBadge = `<span class="badge badge-danger" title="${reasonText}">❌ INVALID</span>`;
                    break;
                default:
                    statusBadge = '<span class="badge badge-warning">⏳ PENDING</span>';
            }
            
            // Add CUTOFF badge if using 2nd eligible day
            if (isCutoff) {
                statusBadge += ' <span class="badge badge-warning" style="margin-left:4px; font-size:0.65rem">⚠️ DAY 2</span>';
            }
            ...
            // ✅ FIX: Only show recharge info if chronologically valid (ticket AFTER recharge)
            let rechargeInfo = '<span style="color:#999;font-size:0.75rem;">—</span>';
            
            try {
                // Check if we have new validation structure with boundRechargeId
                if (validation?.boundRechargeId) {
                    const invalidReason = validation.invalidReasonCode;
                    
                    // Hide recharge info if ticket was before recharge or no recharge exists
                    if (invalidReason === 'INVALID_TICKET_BEFORE_RECHARGE' || invalidReason === 'NO_ELIGIBLE_RECHARGE') {
                        const reasonText = RechargeValidator.getReasonCodeText(invalidReason);
                        rechargeInfo = `<span style="color:#dc3545;font-size:0.70rem;">❌ ${reasonText}</span>`;
                    } else {
                        // Show detailed recharge info with eligibility window
                        const eligibleWindow = validation.eligibleWindow || {};
                        const formatBRTDate = (dateObj) => {
                            if (!dateObj) return '—';
                        ...
                        const eligible1 = eligibleWindow.eligible1 ? formatBRTDate(eligibleWindow.eligible1.day) : '—';
                        const eligible2 = eligibleWindow.eligible2 ? formatBRTDate(eligibleWindow.eligible2.day) : '—';
                        
                        rechargeInfo = `
                            <div style="font-size: 0.70rem; line-height: 1.4;">
                                <div><strong>ID:</strong> ${validation.boundRechargeId.substring(0, 12)}...</div>
                                <div style="margin-top: 2px;"><strong>Amount:</strong> <span class="text-success">R$ ${validation.boundRechargeAmount?.toFixed(2)}</span></div>
                ...
                } else if (validation?.matchedRecharge) {
                    // Fallback for old validation structure
                    const r = validation.matchedRecharge;
                    rechargeInfo = `<span class="text-success" style="font-size:0.75rem;">R$ ${r.amount?.toFixed(2) || '?'}</span>`;
                } else if (validation?.invalidReasonCode) {
                    // Show reason for no recharge
                    const reasonText = RechargeValidator.getReasonCodeText(validation.invalidReasonCode);
                    rechargeInfo = `<span style="color:#dc3545;font-size:0.70rem;">❌ ${reasonText}</span>`;
                }
            } catch (error) {
        ...
        try {
            ...
            if (currentData.validationResults && currentData.validationResults.results) {
                ...
                console.log('UnifiedPage: Sample validation results:', sampleResults.map(v => ({
                    ticket: v.ticket?.ticketNumber || v.ticketNumber,
                    status: v.validity || v.status,
                    hasRecharge: !!(v.boundRechargeId || v.matchedRecharge),
                    rechargeAmount: v.boundRechargeAmount || v.matchedRecharge?.amount,
                    invalidReason: v.invalidReasonCode || v.reason
                })));
            }
