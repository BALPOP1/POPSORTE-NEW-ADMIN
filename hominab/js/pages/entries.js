...
window.EntriesPage = (function() {
    ...
    function renderTable() {
        ...
        tbody.innerHTML = pageEntries.map(entry => {
            ...
            switch (status) {
                case 'VALID':
                    statusBadge = `<span class="badge badge-success" title="${reasonCode}">✅ VALID</span>`;
                    break;
                case 'INVALID':
            ...
            if (isCutoff) {
                statusBadge += ' <span class="badge badge-warning" title="Ticket using 2nd eligible day" style="font-size:0.65rem;">⚠️ DAY 2</span>';
            }
    ...
    function showDetails(ticketNumber) {
            ...
            statusHtml = `
                <div class="status-banner ${statusClass} mb-4">
                    ...
                    <span class="status-banner-text">
                        <strong>${status}</strong> - ${reasonText || 'Checking...'}
                        ${isCutoff ? '<br><span class="text-warning">⚠️ Using 2nd Eligible Day</span>' : ''}
                    </span>
                </div>
            `;
        }
        ...
        // Check new validation structure first
        if (validation?.boundRechargeId) {
            const invalidReason = validation.invalidReasonCode;
            
            // Only show recharge info if chronologically valid
            if (invalidReason !== 'INVALID_TICKET_BEFORE_RECHARGE' && invalidReason !== 'NO_ELIGIBLE_RECHARGE') {
                const formatBRTDateTime = (dateObj) => {
                    if (!dateObj) return '—';
                    const d = new Date(dateObj);
                    return AdminCore.formatBrazilDateTime(d, {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
                ...
                const eligibleWindow = validation.eligibleWindow || {};
                const eligible1 = eligibleWindow.eligible1 ? formatBRTDateTime(eligibleWindow.eligible1.day) : '—';
                const eligible2 = eligibleWindow.eligible2 ? formatBRTDateTime(eligibleWindow.eligible2.day) : '—';
                
                rechargeHtml = `
                    <div class="ticket-info-grid">
                        <div class="ticket-info-item">
                            <span class="label">Recharge ID</span>
                            <span class="value">${validation.boundRechargeId}</span>
                        </div>
                        <div class="ticket-info-item">
                            <span class="label">Amount</span>
                            <span class="value text-success"><strong>R$ ${validation.boundRechargeAmount?.toFixed(2)}</strong></span>
                        </div>
                        <div class="ticket-info-item">
                            <span class="label">Recharge Time</span>
                            <span class="value">${validation.boundRechargeTime || '—'}</span>
                        </div>
                        <div class="ticket-info-item" style="grid-column: span 2;">
                            <span class="label">Eligibility Window</span>
                            <span class="value">
                                <strong>Eligible Draw 1:</strong> ${eligible1}<br>
                                <strong>Eligible Draw 2:</strong> ${eligible2}
                                ${validation.cutoffFlag ? '<br><span class="badge badge-warning" style="font-size:0.75rem;">Using Day 2</span>' : ''}
                            </span>
                        </div>
                    </div>
                `;
            } else {
                // Show reason why no recharge info
                const reasonText = RechargeValidator.getReasonCodeText(invalidReason);
                rechargeHtml = `<p class="text-danger">❌ ${reasonText}</p>`;
            }
        } else if (validation?.matchedRecharge) {
            // Fallback for old validation structure
            const r = validation.matchedRecharge;
