...
window.UnifiedPage = (function() {
    ...
    async function renderEntries() {
        ...
        // Stats - ✅ COUNT DIRECTLY FROM CSV STATUS COLUMN + CUTOFF FLAG
        ...
        const cutoffCount = entries.filter(e => e.cutoffFlag === true).length;
    ...
    function applyEntriesFilters() {
        ...
        switch (sortBy) {
            ...
            case 'cutoff-yes':
                result.sort((a, b) => {
                    const aCutoff = a.cutoffFlag ? 1 : 0;
                    const bCutoff = b.cutoffFlag ? 1 : 0;
    ...
    function renderEntriesTable() {
        ...
        try {
            ...
            // ✅ USE CUTOFF FLAG FROM VALIDATION (Admin logic - Eligible2 window)
            ...
            if (index === 0) {
                ...
                console.log('   Cutoff Flag:', isCutoff);
                console.log('   Bound Recharge:', entry.boundRechargeId ? 'YES' : 'NO');
            }
            ...
            // Add CUTOFF badge if applicable (uses Eligible2 window)
            if (isCutoff) {
            ...
            // ✅ RECHARGE INFO - USE BOUND RECHARGE FROM VALIDATION
            let rechargeInfo = '<span class="text-muted" style="font-size:0.85rem">No recharge bound</span>';
            
            try {
                // Use the bound recharge data from validation
                if (entry.boundRechargeId) {
                    // ORDER NUMBER from bound recharge
                    ...
                    // CHARGE AMOUNT from bound recharge
                    const chargeAmount = entry.boundRechargeAmount || 0;
                    const amountDisplay = `R$ ${chargeAmount.toFixed(2)}`;
                    
                    // TIME from bound recharge
                    let timeDisplay = '-';
                    if (entry.boundRechargeTime) {
                        // boundRechargeTime is a string from the CSV
                        timeDisplay = entry.boundRechargeTime;
                    }
                    ...
                    // Debug first entry with bound recharge
                    if (index === 0) {
                        console.log('💰 BOUND RECHARGE FOR FIRST ENTRY:');
                    ...
                    console.log('❌ NO BOUND RECHARGE for entry:', entry.gameId);
                }
            } catch (error) {
                console.error('❌ ERROR getting bound recharge info:', error);
    ...
    function showTicketDetails(ticketNumber) {
        ...
        const cutoffWarning = isCutoff ? '<br><span class="text-warning">⚠️ Recharge before 20:00, ticket after 20:00 - shifted to next draw window</span>' : '';
        ...
        // ✅ RECHARGE INFORMATION - USE BOUND RECHARGE FROM VALIDATION
        ...
        if (entry.boundRechargeId) {
            ...
            const chargeAmount = entry.boundRechargeAmount || 0;
            const amountDisplay = `R$ ${chargeAmount.toFixed(2)}`;
            const timeDisplay = entry.boundRechargeTime || '-';
            ...
            rechargeHtml = `
                <div class="ticket-info-grid">
                    ...
                    <div class="ticket-info-item">
                        ...
                        <span class="value" style="font-size:0.85rem;word-break:break-all">${orderNumber}</span>
                    </div>
                    <div class="ticket-info-item">
